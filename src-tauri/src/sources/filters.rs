...
fn sort_games<T: AsRef<UnifiedGame>>(games: &mut [T], p: &QueryParams) {
    let sort = p.sort.as_deref().unwrap_or("relevance");
    let order = p.order.as_deref();
    match sort {
        "title" => {
            games.sort_by_key(|g| g.as_ref().title.to_lowercase());
            if order == Some("desc") {
                games.reverse();
            }
        }
        "latest" => games.sort_by(|a, b| {
            b.as_ref()
                .added_at
                .unwrap_or(i64::MIN)
                .cmp(&a.as_ref().added_at.unwrap_or(i64::MIN))
        }),
        _ => {}
    }
...
fn balanced_interleave<T: AsRef<UnifiedGame>>(games: Vec<T>) -> Vec<T> {
...
fn build_facets<T: AsRef<UnifiedGame>>(games: &[T]) -> Facets {
    let mut tag_counts: HashMap<String, usize> = HashMap::new();
    for g in games {
        for t in &g.as_ref().genres {
...
/// Shared tail of the finalize pipeline: filter → sort → facets → total →
/// balanced interleave (when requested). Works over owned games (batch path)
/// or references (incremental streaming path, avoiding a full re-clone per
/// partial), so both paths cannot drift.
pub(crate) fn order_and_filter<T: AsRef<UnifiedGame>>(
    games: Vec<T>,
    params: &QueryParams,
) -> (Vec<T>, Facets, usize) {
    let mut filtered: Vec<T> = games
        .into_iter()
        .filter(|g| matches_filters(g.as_ref(), params))
        .collect();
...
pub(crate) fn finalize_pool_cached(
    ...
    order_and_filter(merged, params)
}
