...
pub async fn resolve(url: &str) -> ResolveResult {
    ...
    let resp = match http::fetch(&api, &opts).await {
        Ok(r) => r,
        Err(_) => return not_resolvable(url, "rootz api request failed"),
    };
    if !resp.status().is_success() {
        return not_resolvable(url, &format!("rootz api {}", resp.status().as_u16()));
    }
    let json = match resp.json::<Value>().await {
        Ok(j) => j,
        Err(_) => return not_resolvable(url, "rootz api returned no json"),
    };

    if json.get("success").and_then(|v| v.as_bool()) != Some(true) {
        return not_resolvable(url, "rootz api said no");
    }
    let data = match json.get("data").filter(|v| v.is_object()) {
        Some(d) => d,
        None => return not_resolvable(url, "rootz api returned no data"),
    };
    if data.get("status").and_then(|v| v.as_str()) != Some("active") {
        return not_resolvable(url, "rootz file is not active");
    }
    if !data
        .get("downloadAllowed")
        ...
        .unwrap_or(false)
    {
        return not_resolvable(url, "rootz download not allowed");
    }
    if data.get("passwordProtected").and_then(|v| v.as_bool()) == Some(true) {
        return not_resolvable(url, "rootz file is password protected");
    }

    let file_id = match data.get("fileId").and_then(|v| v.as_str()) {
        Some(f) => f.to_string(),
        None => return not_resolvable(url, "rootz api returned no file id"),
    };
