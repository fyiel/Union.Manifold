...
async fn live_gamebounty_detail_prunes_dead_mirrors() {
    // tiny-terraces' container lists seven mirrors, but datanodes/vikingfile/
    // filemirage answer 404 and rootz reports status "deleted" — only
    // fileditch/fileq/gofile are alive. Detail must prune the dead ones.
