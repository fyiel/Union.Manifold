...
/// Fold a source-resolved record into a library entry. The card renders
/// `covers`, so a newly-found image must rebuild the candidate list too —
/// patching only `image` leaves the dead thumbnail on screen.
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      const targets = installed.filter((g) => {
        // Local imports with no usable name can never match a source — skip
        // the perf cost; ones with a real name (archive installs) do resolve.
