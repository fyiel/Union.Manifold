...
Nexus, Workshop and Thunderstore installs share a per-mod deployment planner. It recognizes game-relative archive trees, BepInEx, Mod Engine 3, Lenny's Mod Loader, MelonLoader, Bethesda Data folders, Unreal Engine layouts, structured Mods folders, REFramework autorun scripts and Fluffy packages. Unreal Engine support is structural rather than a title list: the planner finds the game's project folder (the one holding `Content/Paks` or `Binaries/Win64`, never `Engine`), sends `.pak`, `.utoc` and `.ucas` packages to `<project>/Content/Paks/~mods`, installs UE4SS next to the shipping binary, and gives a bare UE4SS script mod its own folder under `<project>/Binaries/Win64/ue4ss/Mods`. Archives that already mirror the game root or the project folder are kept as they are, and Windows-style entry names (`ue4ss\Mods\...`) are rebuilt into real directories. The Mods page reports compatible loaders from official Steam title IDs and installed game files: Mod Engine 3 is limited to its five supported FromSoftware titles, Lenny's Mod Loader to Grand Theft Auto V Legacy and Red Dead Redemption 2, MelonLoader to detected Windows Unity games, Fluffy to known titles or RE Engine/MT Framework layouts, and UE4SS to installs that ship an Unreal binary folder, with the reason naming whether the loader is already installed.
